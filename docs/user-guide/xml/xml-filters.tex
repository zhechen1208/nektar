...
The Reynolds stress fields are represented by same the 
polynomial order, P, as the velocity fields defined in the session file.
Since the Reynolds stress fields are quadratic products of the velocity fields, 
(i.e. require twice the polynomial order as the velocity fields), they may be 
under resolved.

To overcome this issue, the user can specify ScaleNumModes, a factor which scales
the number of modes (ScaleNumModes * NUMMODES) of which the Reynolds stresses will
be processed on.
Note that the resulting number of modes is rounded to the lowest integer,
i.e. Nummodes of RSS = floor(3 * 1.4) = 4.
...
\begin{lstlisting}[style=XMLStyle,gobble=2]
    <FILTER TYPE="ReynoldsStresses">
        ...
        <PARAM NAME="OutputFrequency">100</PARAM>
        <PARAM NAME="SampleFrequency"> 10 </PARAM>
        <PARAM NAME="ScaleNumModes"> 1.4 </PARAM>
    </FILTER>
\end{lstlisting}
...
\subsection{Python script}\label{filters:MaxMinFields}

\begin{notebox}
  This feature is only supported if Python support is enabled with the
  \inltt{NEKTAR\_BUILD\_PYTHON} flag enabled at configuration time.
\end{notebox}

The aim of this filter is to permit the execution of a Python script to
implement more complex programmatic filters, but with the ease-of-use of the
Python environment. Python filters mimic the action of C++ filters, and thus
have three separate phases that should be implemented:
\begin{itemize}
  \item an initialisation phase, which is called when the filter is constructed;
  \item an update phase, which is called typically at the end of a timestep;
  \item a finalise phase, which is called at the end of the simulation.
\end{itemize}

Given a Python file as input, the filter can operate in one of two ways:

\begin{itemize}
  \item If only the Python file is supplied, then \nekpp will try to call three
  Python functions: \inltt{filter\_initialise}, \inltt{filter\_update} and
  \inltt{filter\_finalise}. Each function is passed a list of the
  \inltt{ExpList} fields, and the value of the solver time $t$.
  \item If, in addition to the Python file, a filter name is supplied, then
  \nekpp assumes that your Python file creates a subclass of the \inltt{Filter}
  class which has been registered with the \inltt{Filter} factory.
\end{itemize}
...
\begin{center}
  \begin{tabularx}{0.99\textwidth}{lllX}
    ...
    \textbf{Description} \\
    \midrule
    \inltt{PythonFile} & \cmark   & - &
    Name of the Python file to load.\\
    \inltt{FilterName} & \xmark   & - &
    Optional name of filter if Python subclass is implemented.\\
    \bottomrule
  \end{tabularx}
\end{center}
...
\begin{lstlisting}[style=XMLStyle]
  <FILTERS>
    <!-- Python filter with function approach -->
    <FILTER TYPE="Python">
      <PARAM NAME="PythonFile"> FilterPython_Function.py </PARAM>
    </FILTER>
    <!-- Python filter with class approach -->
    <FILTER TYPE="Python">
      <PARAM NAME="PythonFile"> FilterPython_Class.py </PARAM>
      <PARAM NAME="FilterName"> TestFilter </PARAM>
    </FILTER>
  </FILTERS>
\end{lstlisting}
...
class TestFilter(Filter):
    def __init__(self, session, eqsys, params):
        ...
        super().__init__(session, eqsys)
        # Counter which is incremented at each timestep.
        self.num = 0
...
Filter.Register("TestFilter", TestFilter)
\end{lstlisting}
...
The \inltt{FilterLagrangianPoints} filter tracks Lagrangian points in parallel. The following parameters are supported:
\begin{center}
    \begin{tabularx}{0.99\textwidth}{lllX}
        \toprule
        \textbf{Option name} & \textbf{Required} & \textbf{Default} &
